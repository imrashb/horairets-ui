...
function ParametresGenerationToast({
  readyToGenerate,
}: ParametresGenerationToastProps): null {
  ...
  const nombreCoursGeneration =
    nombreCours || Math.min(cours?.length || 0, NOMBRE_MAX_COURS_PAR_HORAIRE);
  ...
  const Component = (
    <>
      ...
      {`${t(
        "nombreCoursParHoraire"
      )}: ${nombreCoursGeneration} ${t("cours").toLowerCase()}`}
      <br />
      {`${t("joursConges")}: ${
        conges?.map((c) => t(c))?.join(", ") || t("aucun")
      }`}
    </>
  );
