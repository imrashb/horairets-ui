...
export const useGetCombinaisonsFromId = () => useMutation({
  mutationFn: async (combinaisonIds: string | string[]) => {
    const params = new URLSearchParams();
    params.append(IDS, Array.isArray(combinaisonIds) ? combinaisonIds.join(',') : combinaisonIds);
    const { data } = await axios.get<Combinaison[]>(
...
export const useGetCours = (programmes?: string[], enabled = true) => useQuery({
  ...
  queryFn: async () => {
    ...
    const url = queryString ? `${GET_COURS_ENDPOINT}?${queryString}` : GET_COURS_ENDPOINT;
